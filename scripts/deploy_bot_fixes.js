...
const filesToUpload = [
  ...
  {
    local: path.join(__dirname, '..', 'src', 'services', 'telegramBot.js'),
    remote: '/var/www/vspaypsp/src/services/telegramBot.js'
  }
];
  ...
  conn.sftp((err, sftp) => {
    ...
    function uploadNext() {
      if (idx >= filesToUpload.length) {
        console.log('\nAll files uploaded. Reloading PM2...');
        conn.exec('cd /var/www/vspaypsp && pm2 reload gaurpay-api --update-env', (err, stream) => {
          if (err) throw err;
